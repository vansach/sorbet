...
class FlattenWalk {
    ...
    ast::ClassDef::RHS_store addClassDefMethods(core::Context ctx, ast::ClassDef::RHS_store rhs, core::Loc loc) {
        ...
        ast::ClassDef::RHS_store new_rhs;
        ...
        for (auto &body : nestedBlocks) {
            ...
            new_rhs.emplace_back(std::move(classDef));
        }
        for (auto &elem : rhs) {
            if (!ast::isa_tree<ast::EmptyTree>(elem.get())) {
                new_rhs.emplace_back(std::move(elem));
            }
        }
