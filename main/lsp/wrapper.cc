...
vector<unique_ptr<LSPMessage>> LSPWrapper::getLSPResponsesFor(unique_ptr<LSPMessage> message) {
    if (multithreadingEnabled) {
        Exception::raise("getLSPResponsesFor is only possible in singlethreaded mode. Use send instead.");
    }
...
unique_ptr<LSPWrapper> LSPWrapper::createMultiThreaded(function<void(unique_ptr<LSPMessage>)> &&processMessage,
                                                       ...
                                                       int numWorkerThreads, bool disableFastPath) {
    return createInternal(rootPath, move(options), move(processMessage), numWorkerThreads, disableFastPath);
}
